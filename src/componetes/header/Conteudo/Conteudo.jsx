...
import { localizacaoimg, login, zap, imgLogout, logo } from "../../../config/imagem";
import { localizacao1 } from "../../../config/localidade";

export default function ConteudoHeader() {
    const navigate = useNavigate()
    const { data, loading, user, logout } = useContext(AppContext);
    const [localAtual, setLocalAtual] = useState(0)
    const [mostrarTodos, setMostrarTodos] = useState(false)

    useEffect(() => {
        if (!localizacao1?.localidade || localizacao1.localidade.length <= 1) return;

        const intervalo = setInterval(() => {
            setLocalAtual((prev) =>
                prev === localizacao1.localidade.length - 1 ? 0 : prev + 1
            )
        }, 3000)
            ...
            <div className="max-w-7xl mx-auto p-4 md:px-10 flex justify-between items-center">
                <div className="flex flex-1 basis-0 justify-start">
                    <div className="w-full max-w-max md:max-w-40">
                        <div className="bg-white border border-gray-200 p-1.5 md:px-3 rounded-lg md:rounded-md flex w-full items-center justify-between cursor-pointer transition-all shadow-sm" >
                            <img className="h-5 w-5 shrink-0" src={localizacaoimg} alt="localização" />
                            <div className="hidden md:flex flex-1 items-center justify-center overflow-hidden ml-2">
                                <span className="text-[15px] font-bold truncate text-gray-800">...</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex items-center flex-1 basis-0 justify-end"></div>
            </div>
        </div>
    );
            ...
            <div className="max-w-7xl mx-auto p-4 md:px-10 flex justify-between items-center">
                ...
                <div className="flex items-center flex-1 basis-0 justify-end gap-2 md:gap-4">
                    ...
                    <div className="flex items-center md:border-l border-gray-200 pl-2 md:pl-4 gap-2">
                        <a title="Matricule-se pelo whatsapp" target="_blank" rel="noreferrer" href={data.matricula} className="hover:scale-110 transition-transform active:scale-90" >
                            <img className="h-7 w-7 md:h-8 md:w-8" src={zap} alt="whastapp" />
                        </a>

                        <div className="flex items-center gap-4">
                            {user ? (
                                <button title="Sair" onClick={fazerLogout} type="button" className="cursor-pointer hover:opacity-80 transition-opacity p-0.5 border-2 border-transparent hover:border-red-100 rounded-full" >
                                    <img className="h-9 w-9 bg-gray-50 rounded-full object-cover p-1" src={imgLogout} alt="logout" />
                                </button>
                            ) : (
                                <button title="Login" onClick={irlogin} type="button" className="cursor-pointer hover:opacity-80 transition-opacity p-0.5 border-2 border-transparent hover:border-blue-100 rounded-full" >
